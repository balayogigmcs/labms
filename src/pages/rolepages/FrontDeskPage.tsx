...
const FrontdeskDashboard: React.FC = () => {
  ...
  const handleAction = async (action: "approved" | "rejected") => {
    ...
    try {
      ...
      let counterRef;
      let fieldToUpdate = ""; // Field to update
      let newCount = 0; // Track the new counter value
  
      if (action === "approved") {
        // Step 1: Determine the correct counter & field name
        if (selectedForm.formType === "micro") {
          counterRef = doc(db, "counters", "micro_om_counter"); // Micro counter
          fieldToUpdate = "om"; // Field in document
        } else if (selectedForm.formType === "chemistry") {
          counterRef = doc(db, "counters", "chemistry_bc_counter"); // Chemistry counter
          fieldToUpdate = "bc"; // Field in document
        } else {
          console.error("Unknown form type:", selectedForm.formType);
          return;
        }
  
        // Step 2: Check if counter document exists
        const counterSnap = await getDoc(counterRef);
        if (!counterSnap.exists()) {
          // If it doesn't exist, create it with initial count = 1
          ...
          newCount = updatedCounterSnap.exists() ? updatedCounterSnap.data().count : 1;
        }
  
        // Step 3: Update the form with the correct field
        await updateDoc(formRef, {
          status: action,
          reviewComment,
          reviewedBy: "Frontdesk",
          reviewedAt: new Date(),
          [fieldToUpdate]: newCount, // Dynamically update `om` or `bc`
        });
      } else {
        // If rejected, update status without modifying counters
        await updateDoc(formRef, {
          status: action,
          reviewComment,
          reviewedBy: "Frontdesk",
        ...
        prevForms.map((form) =>
          form.id === selectedForm.id
            ? {
                ...
                status: action,
                reviewComment,
                [fieldToUpdate]: action === "approved" ? newCount : form[fieldToUpdate],
              }
            : form
        )
      );
